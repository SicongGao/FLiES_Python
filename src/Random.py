        ...
        if (CNT > self.N_RAND):
            for i in range(1, int(self.N_RAND / 100) + 1):
                ...
                for j in range(1 + k + MOVE, k + 100 + MOVE + 1):
                    RandArray[j] = RandArray[j - 250] ^ RandArray[j - 103]
                    # print("randArray["+str(j)+"]:",self.randArray[j])

            for j in range(-249 + MOVE, 0 + MOVE + 1):
                RandArray[j] = RandArray[j + self.N_RAND]
                CNT = 1

        randomValue = RandArray[CNT + MOVE] * self.A_NORM
        logging.debug("randomValue = " + str(randomValue))
        CNT += 1
