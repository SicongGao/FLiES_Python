...
class Parameters:
    ...
    def __init__(self):
        ...
        self.T_FPR = self.C_FPR = self.B_FPR = self.F_FPR = self.S_FPR = 0.0
        ...
        self.FF_DIF = np.zeros(comm.SIZE * comm.SIZE, dtype=float).reshape(comm.SIZE, comm.SIZE)
        self.SF_DIR = np.zeros(comm.SIZE * comm.SIZE, dtype=float).reshape(comm.SIZE, comm.SIZE)
        self.SF_DIF = np.zeros(comm.SIZE * comm.SIZE, dtype=float).reshape(comm.SIZE, comm.SIZE)

        self.PROC_F = np.zeros((comm.K_NXR + 1) * (comm.K_NXR + 1) * comm.K_NRDC, dtype=float).reshape((comm.K_NXR + 1), (comm.K_NXR + 1), comm.K_NRDC)
        self.PROC_Q = np.zeros((comm.K_NXR + 1) * (comm.K_NXR + 1) * comm.K_NRDC, dtype=float).reshape((comm.K_NXR + 1), (comm.K_NXR + 1), comm.K_NRDC)

        self.BRF = np.zeros(3 * 700, dtype=float).reshape(3, 700)
        self.BRF_C = np.zeros(3 * 700, dtype=float).reshape(3, 700)
        self.BRF_S = np.zeros(3 * 700, dtype=float).reshape(3, 700)
        self.BRF_F = np.zeros(3 * 700, dtype=float).reshape(3, 700)

        self.REFL = np.zeros(3 * comm.SIZE * comm.SIZE, dtype=float).reshape(3, comm.SIZE, comm.SIZE)
        self.I_REFL = np.zeros(3 * comm.SIZE * comm.SIZE, dtype=int).reshape(3, comm.SIZE, comm.SIZE)
    ...
    def writeData(self):
        ...
        for i in range(1, self.nwl + 1):
            self.scmpf[1, i] *= 100.0 / self.tflx
            self.scmpf[2, i] *= 100.0 / self.bflx
            self.scmpf[3, i] *= 100.0 / self.dflx
            self.scmpp[1, i] *= 100.0 / self.tpfd
            self.scmpp[2, i] *= 100.0 / self.bpfd
            self.scmpp[3, i] *= 100.0 / self.dpfd
        ...
        # fraction of absorbed radiation (far)
        self.C_FPR *= self.RQ * abs(self.cos_q0) / float(self.nPhoton)
        self.B_FPR *= self.RQ * abs(self.cos_q0) / float(self.nPhoton)
        self.F_FPR *= self.RQ * abs(self.cos_q0) / float(self.nPhoton)
        self.S_FPR *= self.RQ * abs(self.cos_q0) / float(self.nPhoton)
        self.T_FPR = self.C_FPR + self.B_FPR + self.F_FPR

        self.T_FPR /= self.tpfd
        self.C_FPR /= self.tpfd
        self.B_FPR /= self.tpfd
        self.F_FPR /= self.tpfd
        self.S_FPR /= self.tpfd
        ...
        for k in range(int(comm.Z_MAX) + 1, 0, -1):
            for i in range(1, comm.SIZE + 1):
                for j in range(1, comm.SIZE + 1):
                    self.AP[i, j, k] *= self.RQ * abs(self.cos_q0) / pixelNP
                    self.AP_D[i, j, k] *= self.RQ * abs(self.cos_q0) / pixelNP
                    self.AP_B[i, j, k] *= abs(self.cos_q0) / pixelNP
                    self.AP_B[i, j, k] *= (1.0 / comm.GT_BLC[ith])
                    self.AP_B[i, j, k] = min(self.AP_B[i, j, k], 1.0)
                    app[k] += self.AP[i, j, k]

            app[k] /= (self.tpfd * float(comm.SIZE ** 2))
            self.AP_NP[k] *= self.RQ * abs(self.cos_q0) / float(self.nPhoton) / self.tpfd

        # summary of forest floor far etc
        for i in range(1, comm.SIZE + 1):
            for j in range(1, comm.SIZE + 1):
                self.AP_F[i, j] *= self.RQ * abs(self.cos_q0) / pixelNP
                self.AP_FD[i, j] *= self.RQ * abs(self.cos_q0) / pixelNP
        ...
        for i in range(1, comm.SIZE + 1):
            for j in range(1, comm.SIZE + 1):
                self.SF_DIR[i, j] *= self.RQ * abs(self.cos_q0) / pixelNP
                self.SF_DIF[i, j] *= self.RQ * abs(self.cos_q0) / pixelNP
                self.FF_DIR[i, j] *= self.RQ * abs(self.cos_q0) / pixelNP
                self.FF_DIF[i, j] *= self.RQ * abs(self.cos_q0) / pixelNP

                ffSum += self.FF_DIR[i, j] + self.FF_DIF[i, j]
                sfSum += self.SF_DIR[i, j] + self.SF_DIF[i, j]
        ...
        f.write(format(self.tflx, '8.5f') + format(self.bflx, '12.5f') + format(self.dflx, '12.5f') +
                format(self.dflx / self.tflx, '12.5f') + "\n")

        f.write("\nPhoton unit (W/m2)"+ "\n")
        f.write(format("Total", '12') + format("Beam", '12') + format("Diffuse", '12') + format("Diff.Ratio", '12') + "\n")
        f.write(format(self.tpfd, '8.5f') + format(self.bpfd, '12.5f') + format(self.dpfd, '12.5f') +
                format(self.dpfd / self.tpfd, '12.5f') + "\n")

        f.write("\nAlbedo (A)"+ "\n")
        f.write(format("Actual", '12') + format("black", '12') + format("white", '12') + "\n")
        f.write(format("", '12') + format("(beam)", '12') + format("(diffuse)", '12') + "\n")
        f.write(format(self.rflx / self.tflx, '8.5f') + format(self.rbflx / self.bflx, '11.5f') +
                format(self.rdflx / self.dflx, '11.5f') + "\n")
        ...
        if (self.surfaceType == 2):
            ...
            f.write(format(self.T_FPR, '10.7f') + format(self.C_FPR, '12.7f') + format(self.B_FPR, '14.7f') +
                    format(self.F_FPR, '13.7f') + "\n")

            f.write("\nVertical profile of far" + "\n")
            f.write(format("H(m)", '12') + format("total", '12') + format("leaf", '12') + format("non-leaf", '8') + "\n")
            string = ""
            for k in range(int(comm.Z_MAX) + 1, 0, -1):
                string += format(k, '5') + format(app[k] + self.AP_NP[k], '12.6f') + format(app[k], '14.6f') + \
        ...
        if (self.cmode == 1):
            ...
            for i in range(1, comm.N_ANG_C + 1):
                self.BRF_C[1, i] /= self.BRF[1, i]
                self.BRF_S[1, i] /= self.BRF[1, i]
                self.BRF_F[1, i] /= self.BRF[1, i]

                if (self.AtmMode == 1):
                    self.BRF[1, i] *= pi / (float(self.nPhoton) * tm)
                else:
                    self.BRF[1, i] *= pi / float(self.nPhoton)

                    self.BRF_C[2, i] /= self.BRF[2, i]
                    self.BRF_S[2, i] /= self.BRF[2, i]
                    self.BRF_F[2, i] /= self.BRF[2, i]
                self.BRF[2, i] *= pi / float(self.nPhoton)
                ...
                for i in range(1, comm.N_TH + 1):
                    if (self.AtmMode == 1):
                        f.write(format(comm.ANG_T[i], '8.2f') + format(comm.ANG_P[j], '8.2f') +
                                format(self.BRF[1, k], '10.6f') + format(self.BRF[2, k], '14.6f') +
                                format(self.BRF_C[1, k], '14.6f') + format(self.BRF_S[1, k], '14.6f') +
                                format(self.BRF_F[1, k], '14.6f') + "\n")
                    else:
                        f.write(format(comm.ANG_T[i], '8.2f') + format(comm.ANG_P[j], '8.2f') +
                                format(self.BRF[1, k], '10.6f') +
                                format(self.BRF_C[1, k], '14.6f') + format(self.BRF_S[1, k], '14.6f') +
                                format(self.BRF_F[1, k], '14.6f') + "\n")
        ...
        if (self.cmode == 3):
            ...
            for k in range(int(comm.Z_MAX) + 1, 0, -1):
                for j in range(comm.SIZE, 0, -1):
                    string = ""
                    for i in range(1, comm.SIZE + 1):
                        string += format(self.AP[i, j, k], '12.5f')
                    f.write(string + "\n")
            f.close()
            ...
            for k in range(int(comm.Z_MAX) + 1, 0, -1):
                for j in range(comm.SIZE, 0, -1):
                    string = ""
                    for i in range(1, comm.SIZE + 1):
                        string += format(self.AP_B[i, j, k], '12.5f')
                    f.write(string + "\n")
            f.close()
